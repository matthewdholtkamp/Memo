...
function createLetterheadSeal(image: { data: Uint8Array; type: "png" | "jpg" }): ImageRun {
  return new ImageRun({
    type: image.type,
    data: image.data,
    transformation: {
      width: ARMY_SEAL_SIZE,
      height: ARMY_SEAL_SIZE
    },
    floating: {
      horizontalPosition: {
        relative: HorizontalPositionRelativeFrom.PAGE,
        offset: ARMY_SEAL_OFFSET
      },
      verticalPosition: {
...
function createLetterhead(spec: MemoSpec): Paragraph[] {
  const image = spec.letterhead.sealImageDataUrl
    ...
    : null;
  const seal = image ? createLetterheadSeal(image) : null;
